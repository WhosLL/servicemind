...
export default function Onboard({ onBack, onComplete }) {
  ...
  const [info, setInfo] = useState({
    ...
    instagram: '', city: '', state: '', address: '',
    salon_type: 'barbershop', passcode: ''
  })
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data: sd, error: se } = await sb().from('salons').insert([{
        ...info, hours, slug,
        subscription_status: 'trial', subscription_tier: 'basic',
        monthly_rate: 150, onboarded: true, onboarded_at: new Date().toISOString()
      }]).select()
      if (se) throw se
      const salon = sd[0]
      ...
      setStep(5)
      setTimeout(() => onComplete(salon), 2200)
  ...
  const inp = (label, key, placeholder, type = 'text') => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input style={{ background: 'var(--dark-3)', border: '1px solid var(--border-dim)', color: 'var(--text)', padding: '14px 18px', fontSize: 13, fontFamily: 'inherit', fontWeight: 300, width: '100%', outline: 'none', borderRadius: 0 }}
        type={type} placeholder={placeholder} value={info[key]}
  ...
  const Progress = () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 44 }}>
      <button onClick={step === 1 ? onBack : () => { setStep(step - 1); setErr('') }} className="btn-ghost" style={{ fontSize: 10, padding: '9px 18px' }}>← Back</button>
      <div style={{ display: 'flex', gap: 5, flex: 1 }}>
        {[1, 2, 3, 4].map(i => <div key={i} style={{ height: 2, flex: i <= step ? 1 : '.4', background: i <= step ? 'var(--gold)' : 'var(--border-dim)', transition: 'all .3s' }} />)}
      </div>
  ...
  const Wrap = ({ title, italic, sub, children, onNext, nextLabel = 'Continue →', canNext = true }) => (
  ...
  if (step === 1) return (
    <Wrap title="Your Shop" italic="Info." sub="This powers your website, booking page, and AI automatically."
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>{inp('Shop Name *', 'shop_name', 'e.g. Boo Cutz')}</div>
        <div>{inp('Owner Name *', 'owner_name', 'e.g. Brandon McCoy')}</div>
        <div>{inp('Phone *', 'phone', '(404) 000-0000', 'tel')}</div>
        <div>{inp('Email', 'email', 'you@shop.com', 'email')}</div>
        <div>{inp('Instagram', 'instagram', '@yourshop')}</div>
        <div>
          ...
          <select style={{ background: 'var(--dark-3)', border: '1px solid var(--border-dim)', color: 'var(--text)', padding: '14px 18px', fontSize: 13, fontFamily: 'inherit', width: '100%', outline: 'none', borderRadius: 0 }}
            value={info.salon_type} onChange={e => setInfo({ ...info, salon_type: e.target.value })}>
            {TYPES.map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
          </select>
        </div>
        <div>{inp('City', 'city', 'Raleigh')}</div>
        <div>{inp('State', 'state', 'NC')}</div>
      </div>
      {inp('Address (optional)', 'address', '123 Main Street')}
    </Wrap>
  )

  if (step === 2) return (
    <Wrap title="Your" italic="Services." sub="Edit names and prices — these populate your booking page instantly."
                  ...
                  <div key={idx} style={{ background: 'var(--dark-3)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, border: '1px solid var(--border-dim)', marginBottom: 2 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                      ...
                      <input style={{ background: 'var(--dark-2)', border: '1px solid var(--border-dim)', color: 'var(--gold)', padding: '6px 10px', fontSize: 13, width: 72, textAlign: 'center', outline: 'none', fontFamily: 'inherit', borderRadius: 0 }}
                        ...
                        onChange={e => { const u = [...svcs]; u[idx] = { ...u[idx], price: +e.target.value }; setSvcs(u) }} />
                    </div>
                  </div>
                )
              })}
  ...
  if (step === 3) return (
    <Wrap title="Hours &" italic="Passcode." sub="Set your available hours and create a secure passcode for your dashboard."
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        {Object.entries(DAYS).map(([day, label]) => (
          <div key={day} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', background: 'var(--dark-3)', border: '1px solid var(--border-dim)', marginBottom: 2 }}>
            <input type="checkbox" checked={!!hours[day]} onChange={e => setHours({ ...hours, [day]: e.target.checked ? { open: '8:00 AM', close: '6:00 PM' } : null })} />
            ...
            {hours[day] ? (
              <>
                <select style={{ background: 'var(--dark-2)', border: '1px solid var(--border-dim)', color: 'var(--text)', padding: '7px 10px', fontSize: 11, outline: 'none', borderRadius: 0, flex: 1 }}
                  value={hours[day].open} onChange={e => setHours({ ...hours, [day]: { ...hours[day], open: e.target.value } })}>
                  {OPENS.map(t => <option key={t}>{t}</option>)}
                </select>
                <span style={{ fontSize: 11, color: 'var(--muted)', flexShrink: 0 }}>to</span>
                <select style={{ background: 'var(--dark-2)', border: '1px solid var(--border-dim)', color: 'var(--text)', padding: '7px 10px', fontSize: 11, outline: 'none', borderRadius: 0, flex: 1 }}
                  value={hours[day].close} onChange={e => setHours({ ...hours, [day]: { ...hours[day], close: e.target.value } })}>
                  {CLOSES.map(t => <option key={t}>{t}</option>)}
                </select>
              </>
            ) : (
              <span style={{ fontSize: 11, color: 'var(--muted)', letterSpacing: '.1em' }}>Closed</span>
            )}
          </div>
        ))}
      </div>
      <div>
        ...
        <input style={{ background: 'var(--dark-3)', border: '1px solid var(--border-dim)', color: 'var(--text)', padding: '15px 18px', fontSize: 14, fontFamily: 'inherit', fontWeight: 300, width: '100%', outline: 'none', borderRadius: 0 }}
          type="password" placeholder="Minimum 4 characters" value={info.passcode}
          onChange={e => setInfo({ ...info, passcode: e.target.value })} />
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--black)', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 520 }}>
        ...
        <h2 className="cormorant" style={{ fontSize: 60, fontWeight: 300, marginBottom: 20, lineHeight: 1.05 }}>
          {info.shop_name}<br />
          is <em style={{ color: 'var(--gold)', fontStyle: 'italic' }}>ready.</em>
        </h2>
        <p style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.9 }}>
          Your website, 8 automations, and full dashboard are live. Opening your dashboard now...
        </p>
      </div>
    </div>
  )
